...
export default function MyApp({ Component, pageProps }: AppProps<MyAppProps>) {
  let sideMenu = [];
  const [showSearchModal, toggleSearchModal] = React.useState(false);

  const childToParent = () => {
    toggleSearchModal((m) => !m)
  }
  ...
  return (
    <>
      ...
      <div className="flex">
        {/* <SideNav /> */}
        <TableOfContents  childToParent={childToParent}  toc={sideMenu} />
        <main className="w-screen lg:pr-[220px]">
          <Navbar />
          <Component {...pageProps} />
        </main>
      </div>
      {showSearchModal && <SearchModal closeSearchModal={ ()=> toggleSearchModal((m) => !m)}/>}
